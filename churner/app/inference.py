...
class Predictor:
    ...
    def preprocess(self, input_dict):
        ...
        # Separate variables
        X = preprocessor(
            df, self.config, option_train="inference", option_output="inference"
        )
    ...
    def predict(self, X):
        ...
        model_path = os.path.join(
            self.config["model"]["savepath"], self.config["model"]["name"]
        )
    ...
    def postprocess(self, proba):
        ...
        response = {"message": message, "probability": round(proba, 4)}
        return response
